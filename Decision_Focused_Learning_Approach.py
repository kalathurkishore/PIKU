...
from Modeling_Decision_focused_Learning import *
...
npzfile = np.load("/DATA/kumar278/CVRP2D/data/daily_stops_data.npz",allow_pickle=True,)
...
pickups = np.load("/DATA/kumar278/CVRP2D/data/pickup_list_data.npy")  # pickups at each active stops
npzfile = np.load("/DATA/kumar278/CVRP-DP/Data-Driven-VRP/data/daily_routematrix_data.npz", allow_pickle=True)
opmat = npzfile['incidence_matrices'] # solutions for each day as an incidence matrix
stop_wise_days = npzfile['stop_wise_active'] 
distance_mat = np.load("/DATA/kumar278/CVRP2D/data/Distancematrix.npy")
edge_mat = np.load("/DATA/kumar278/CVRP2D/data/edge_category_data.npy")
...
for loss_type in ["Relu","Squared"]:
        ...
        for t in test_days:
                ...
                n_vehicles[[t]],opmat[:(t+1)],stop_wise_days,demands[t], pickups[t],capacities[t:(t+1)],
                capacitated=True)
