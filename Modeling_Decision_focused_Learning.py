...
from Constraint_Solver_using_Gurobi import VRPGurobi, eval_ad, eval_sd,VRPsolutiontoList
import datetime
import logging
...
class PredictOptimzeVRP:
        ...
        for ep in range(self.epochs):
            ...
            for d in range(training_days):
                ...
                if self.capacitated:
                    ...
                    qpickups = pickups[d + self.lookback_period]
                ...
                if self.loss_func=="BB":
                    solved,cmnt, sol_hat,u = VRPGurobi(proba_mat_np,qcapacity,qpickups,Q,
                    ...
                    if solved:
                        ...
                        solved,cmnt, sol_tilde,u = VRPGurobi(proba_tilde,qcapacity,qpickups,Q,
                    n_vehicleslist[(d + self.lookback_period) ] ,
                    stops_list[d + self.lookback_period], relaxed= self.relaxed )
                        sol_tilde =  np.rint(sol_tilde)
                        grad = torch.from_numpy((sol_tilde - sol_hat)/self.mu).float()
                if self.loss_func=="SPO":
                    solved,cmnt, sol_hat,u = VRPGurobi(proba_mat_np+self.mu,qcapacity,qpickups,Q,
                    n_vehicleslist[(d + self.lookback_period) ] ,
                    stops_list[d + self.lookback_period] )
                    if solved:
                        sol_hat =  np.rint(sol_hat)
    ...
    def evaluation(self,distance_mat,stops_list,weekday,n_vehicleslist,
        trgt, active_days,demands,pickups, capacities, capacitated =True):
        '''
