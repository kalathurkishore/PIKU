...
from Constraint_Solver_using_Gurobi import VRPGurobi, VRPsolutiontoList, eval_ad, eval_sd  
from Util_SPD import *
'''
With week day
'''
class TwoStageVRP:
    ...
    def evaluation(self,distance_mat,stops_list,weekday,n_vehicleslist,
        trgt, active_days,demands, pickups, capacities, capacitated =True, omega=1):
        '''
        ...
        if capacitated:
            qcapacity = demands
            qpickups = pickups #added part pickup (if case)
            Q = capacities[0]            
        else:
            qcapacity = np.ones(74)
            qpickups = np.ones(74) #added part pickup (else case)
            Q = len(activeindices)
        solved,cmnt, sol,u = VRPGurobi(proba_mat,qcapacity,qpickups,Q,
        n_vehicleslist[-1] ,activeindices)
        '''
        #added part
        object = VRPSDP_GUROBI(proba_mat,qcapacity,qpickups,n_vehicleslist[-1],Q)#initializing VRPSDP_GUROBI
        k=object.solve()
        ...
        if solved:
            #solu = np.rint(object)
            P1 = VRPsolutiontoList(object)
            A1 = VRPsolutiontoList(act)
        else:
            raise Exception("VRP not solved for day {}".format(len(trgt_past)))
        return eval_ad (P1,A1), eval_sd(P1,A1),bceloss,\
        self.training_loss,np.sum(distance_mat*solu), cmnt
        ########
        '''
        #added previous part
        if solved:
