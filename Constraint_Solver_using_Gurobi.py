...
def VRPGurobi(cst_matrix,qcapacity,qpickups,Q,n_vehicles,activeindices, relaxed=False,gap=1e-2):
    '''
    ...
    n = len(cst_matrix)
    M = Model("VRP-SDP")#changed from vrp to VRP-SDP
    XRP, R, P = [], [], []
    ...
    #########
    #adding code regarding to R and P
    for i in range(n):
        XRPRow, RRow, PRow = [], [], []
        for j in range(n):
            XRPRow.append(M.addVar(name='x('+str(i)+","+str(j)+")", vtype=GRB.BINARY, lb=0, ub=1))
            RRow.append(M.addVar(name='R('+str(i)+","+str(j)+")", vtype=GRB.INTEGER, lb=0))#scalar variable wrt to Request(Delivery)
            PRow.append(M.addVar(name='P('+str(i)+","+str(j)+")", vtype=GRB.INTEGER, lb=0))#scalar variable wrt to Pickup
        XRP.append(XRPRow)
        R.append(RRow)
        P.append(PRow)
    ##########
    ...
    '''
    n is the number of rows in the matrix of variables to be added
    ub is the number of columns in the matrix of variables
    '''
    M.addConstrs(x[i,i]==0 for i in range(n)) # constraint ensures that the diagonal elements of the matrix variable x are 0
    M.addConstr(u[0]==0) #first element of the variable vector u (u[0]) must be equal to 0

    M.addConstrs(sum(x[:,i] )==1 for i in activeindices if i>0 ) #The constraints created are that for each index i in the set of active indices, if i > 0, the sum of the i-th column of the matrix variable x (sum(x[:,i])) must be equal to 1.
    M.addConstrs(sum(x[i,:] )==1 for i in activeindices if i>0) #The constraints created are that for each index i in the set of active indices, if i > 0, the sum of the i-th row of the matrix variable x (sum(x[i,:])) must be equal to 1.
...
#     M.addConstr(sum(x[:,0]) <= n_vehicles )
    ...
    ###########
    # Adding pickup and delivery constraints
    # constraint 3
    for j in range(1,n):
        ...
        M.addConstr(const3a - qcapacity[j] == const3b)
# constraint 4
    for j in range(1,n):
        ...
        M.addConstr(const4a + qpickups[j] == const4b)
# constraint 5
    const5 = None
    for i in range(1,n):
        if(const5 == None):
    ...
    for i in range(1,n):
        if(const6 == None):
            ...
            const6 = const6 + R[i][0]
    M.addConstr(const6 == 0)

    # constraint 7
    for i in range(n):
        for j in range(n):
            M.addConstr(R[i][j] + P[i][j] <= Q* XRP[i][j])
    ###########
